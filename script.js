...
class Particle {
   ...
   */
  constructor(x, y, size, colour, initialForce, initalForceDirection, mass, lifespan) {
    this.x = x;
    this.y = y;
  ...
  /* Adds an explosive force onto the particle, 
   * The explosion originates at coords x, y.
   * The further away the current particle is from this, the weaker the force applied.
   */
  explode(force, x, y){
    //First calculate what angle the force will be applied on the particle.
    let yDifference = this.y - y;
    let xDifference = this.x - x;
    let explosionForceAngle = atan2(yDifference, xDifference);
    
    //Decrease the force applied to the particle the further away from the explosion it is.
    let appliedExplosionForce = force*10 / dist(this.x, this.y, x, y);
    
    //Resolve the two forces acting on the particle along the x and y axes.
    let newXForce = cos(this.forceDirection) * this.force + cos(explosionForceAngle) * appliedExplosionForce * this.mass;
    let newYForce = sin(this.forceDirection) * this.force + sin(explosionForceAngle) * appliedExplosionForce * this.mass;
  ...
  display(){
    ...
    if (this.age < this.lifespan/3){ //Fade in on spawn.
      fill(this.colour.h, this.colour.s * 100, this.colour.v * 100, this.age/this.lifespan); 
      //"* 100" for sat and val as dat.GUI wants 0 - 1 while p5js wants 0 - 100
    } else if (this.age < this.lifeSpan * 2/3){ //Full opacity in center of life.
      fill(this.colour.h, this.colour.s * 100, this.colour.v * 100, 1);
    } else { //Fade out on death.
      fill(this.colour.h, this.colour.s * 100, this.colour.v * 100, 1-this.age/this.lifespan);
    }
...
class ParticleSimulation {
   ...
   */
  constructor(spawnX, spawnY, darkness){
    ...
    this.maxAdditionalInitialForce = 2.5;
    this.initialDirection = PI + HALF_PI;
    this.directionFocus = PI;
    this.minDarkness = darkness;
    this.minSize = 8;
    this.sizeVariation = 8;
    this.minLifespan = 50;
    this.maxAdditionalLifespan = 250;
    this.mouseClickForce = 50;
   ...
   */
  setupGUI(gui){
    gui.add(this, 'numParticles', 1, 500).name("Particles");
    ...
    gravityFolder.add(this, 'gravityForce', -3, 3).name("Force");
    gravityFolder.add(this, 'gravityDirection', 0, TWO_PI).name("Direction");
    
    let lifeSpanFolder = gui.addFolder("Lifespan");
    lifeSpanFolder.add(this, 'minLifespan', 0, 300).name("Minimum");
    lifeSpanFolder.add(this, 'maxAdditionalLifespan', 0, 300).name("Max Add.");
    
    let clickFolder = gui.addFolder("Mouse Click");
    clickFolder.add(this, 'mouseClickForce', 0, 100).name("Force");
  }
  ...
  createNewParticle(){
    return new Particle(this.spawnX * windowWidth,   //x
                        ...
                        {h: 42, s: 0, v: this.minDarkness + random(0.2)}, //colour
                        ...
                        0.5 + random(1), //mass
                        this.minLifespan + random(this.maxAdditionalLifespan),
                       );
  }
  ...
  explodeParticles(x, y){
    let force = this.mouseClickForce;
    this.particles.forEach(function(particle){
      particle.explode(force, x, y);
    });
  }
...
function setup(){
  ...
  particleSystem = new ParticleSimulation(0.33, 0.5, 0.20);
  particleSystem.setupGUI(gui);
}

function draw(){
  background(0, 0, 10, 0.35); //The 0.35 opacity value gives the particle trail effect.
...
function mousePressed(){
  particleSystem.explodeParticles(mouseX, mouseY);
}
